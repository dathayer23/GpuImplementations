 ...
 *
 * Following kernels support c++ templates but no functors. To get 
 * the mangled kernel name, open the ptx-file with a text editor and
 * look for the lines starting with ".entry"
 */
...
__device__
uchar4 boxfilter_functor(const uchar4 &a, const uchar4 &b, const uchar4 &c, const uchar4 &d) 
{
...
__device__
unsigned char maxfilter_functor(const unsigned char &a, const unsigned char &b, const unsigned char &c, const unsigned char &d) 
{
...
__device__
unsigned char minfilter_functor(const unsigned char &a, const unsigned char &b, const unsigned char &c, const unsigned char &d) 
{
...
template<class T>
__global__
void downscaleKernelBox(T* small_image, int small_pitch, int small_width, int small_height, const T* image, int pitch, int width, int height) {
	...
	for( int k=0; k < 4; ++k ) {
		...
		if( small_y < small_height && small_x0 < small_width ) {
			small_image[small_y * small_pitch + small_x0] = boxfilter_functor( tile[2 * threadIdx.y][2 * threadIdx.x], 
...
template<class T>
__global__
void downscaleKernelMax(T* small_image, int small_pitch, int small_width, int small_height, const T* image, int pitch, int width, int height) {
	__shared__ T tile[16][64];
	...
	for( int k=0; k < 4; ++k ) {
		...
		if( small_y < small_height && small_x0 < small_width ) {
			small_image[small_y * small_pitch + small_x0] = maxfilter_functor( tile[2 * threadIdx.y][2 * threadIdx.x], 
				tile[2 * threadIdx.y][2 * threadIdx.x+1],
				tile[2 * threadIdx.y+1][2 * threadIdx.x+1],
				tile[2 * threadIdx.y+1][2 * threadIdx.x]);
		}
...
cudaError_t downscale(uchar4* small_image, int small_pitch, int small_width, int small_height, const uchar4* image, int pitch, int width, int height) {
	...
	downscaleKernelBox<<<grid, block>>>(small_image, small_pitch/4, small_width, small_height, image, pitch/4, width, height);
...
cudaError_t downscaleTrimap(unsigned char* small_image, int small_pitch, int small_width, int small_height, const unsigned char* image, int pitch, int width, int height) {
	...
	downscaleKernelMax<<<grid, block>>>(small_image, small_pitch, small_width, small_height, image, pitch, width, height);
	return cudaGetLastError();
}
