...
namespace FluidsGLCSharp
{
    public partial class Form1 : Form
    {
        ...
        const int DIM = 800;    // Square size of solver domain
        ...
        const float VIS = 0.025f;          // Viscosity constant
        const float FORCE = (5.8f*DIM);     // Force scale factor 
        const int FR = 8;                   // Force update radius
        ...
        private void initGLAndCuda()
        {
            ...
            planr2c = new CudaFFTPlan2D(DIM, DIM, cufftType.R2C, Compatibility.PADDING);
            planc2r = new CudaFFTPlan2D(DIM, DIM, cufftType.C2R, Compatibility.PADDING);
        ...
        private void DrawPoints(float R, float G, float B, int begin, int end)
        {
            GL.Color4(R, G, B, 0.5f);
            GL.PointSize(1);
            ...
            GL.DrawArrays(BeginMode.Points, begin, end);
            GL.BindBuffer(BufferTarget.ArrayBuffer, 0);
        ...
        int numOfRefractoryPeriods = 100;
        void pulsar()
        {
            int numOfPulses = rnd.Next(2, 15);
            
            if (numOfRefractoryPeriods == 0)
            {
                numOfRefractoryPeriods = rnd.Next(100, 5000);
                lastPulsex = rnd.Next(0, m_renderControl.Width);
                lastPulsey = rnd.Next(0, m_renderControl.Height);
                
                for (int i = 0; i < numOfPulses; i++)
                {
                    int ex = rnd.Next(Math.Min(lastPulsex - 15, 0), Math.Max(lastPulsex + 15, DIM));
                    int ey = rnd.Next(Math.Min(lastPulsey - 15, 0), Math.Max(lastPulsey + 15, DIM));
                    pulse(ex, ey);
                }
            }
            else
            {
                numOfRefractoryPeriods--;
            }
        }

        void pulse(int e_x, int e_y)
        {
            float fx = (lastPulsex / (float)m_renderControl.Width);
            float fy = (lastPulsey / (float)m_renderControl.Height);
            int nx = (int)(fx * DIM);
            int ny = (int)(fy * DIM);
            if (nx < DIM - FR && nx > FR - 1 && ny < DIM - FR && ny > FR - 1)
            {
                int ddx = e_x - lastPulsex;
                int ddy = e_y - lastPulsey;
        ...
        void m_renderControl_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            //Console.WriteLine("Mouse Movement ({0},{1})", e.X,e.Y);
